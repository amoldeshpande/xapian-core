...
OrContext::select_elite_set(size_t set_size, size_t out_of)
{
    ...
    for_each(begin, pls.end(), mem_fn(&PostList::recalc_maxweight));
...
PostList *
AndContext::PosFilter::postlist(PostList * pl, const vector<PostList*>& plsParam) const
try {
    vector<PostList *>::const_iterator terms_begin = plsParam.begin() + begin;
    vector<PostList *>::const_iterator terms_end = plsParam.begin() + end;
